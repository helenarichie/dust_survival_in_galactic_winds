...
##############################################
date = "frontier/2024-03-18"
datestr = "0318"
vlims_gas = (19.2, 21.9) 
vlims_dust = (-9.6, -4)
fnums = [0, 350, 800]     # disrupted
spacing, unit = 320*1e-3, "kpc" # pc, spacing of tick marks
##############################################
...
for i, row in enumerate(ax):
    for j, col in enumerate(row):
        ...
        if j == 0:
            if i == 1:
                ax[i][j].hlines(0.86*dx*ylen, 0.5*spacing, 1.5*spacing, color='white')
                ax[i][j].text(1.5*spacing+0.05, 0.83*dx*ylen, '320 pc', color='white', fontsize=fontsize)
                ax[i][j].text(0.59*spacing, 0.125*dx*ylen, 'dust', color='white', fontsize=fontsize)
        if i == 0:
            ax[i][j].text(0.5*spacing, 0.81*dx*ylen, f'{round(t[0]/1e6, 1)} Myr', color='white', fontsize=fontsize)
            if j == 0:
                ax[i][j].text(0.6*spacing, 0.125*dx*ylen, 'gas', color='white', fontsize=fontsize)

        ax[i][j].set_xticks(np.arange(0, xlen[j]*dx, spacing))
        ax[i][j].set_yticks(np.arange(0, ylen*dx, spacing))
...
plt.savefig(pngdir + f"cloud_dust_snapshots_{datestr}.png", dpi=300, bbox_inches="tight")
plt.close()
