...
for i, dust in enumerate(d_dust_solution):
    if x0 == 0.5 * kpc_in_cm:
        with open(os.path.join(csvdir, "single_cell_wind_shielded.csv"), "a") as f:
            writer_obj = writer(f)
            writer_obj.writerow([t_arr[i], x_distance[i], d_dust_solution[i]])
            f.close()
    if x0 == 0:
        with open(os.path.join(csvdir, "single_cell_wind_unshielded.csv"), "a") as f:
            writer_obj = writer(f)
            writer_obj.writerow([t_arr[i], x_distance[i], d_dust_solution[i]])
            f.close()
