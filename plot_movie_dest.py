...
for i in range(ns, ne+1):
    ...
    # plot
    fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(27.5,9), gridspec_kw={"width_ratios":[4.75, 24], 'wspace':0, 'hspace':0})
    ...
    ax[0][0].tick_params(axis='both', which='both', direction='in', color=mode_color, labelbottom=0, top=1, right=1, length=ticklength, width=tickwidth, labelsize=fontsize-7)
    ax[0][1].tick_params(axis='both', which='both', direction='in', color='white', labelleft=0, labelbottom=0, top=1, right=1, length=ticklength, width=tickwidth, labelsize=fontsize-7)
    ax[1][0].tick_params(axis='both', which='both', direction='in', color=mode_color, top=1, right=1, length=ticklength, width=tickwidth, labelsize=fontsize-7)
    ax[1][1].tick_params(axis='both', which='both', direction='in', color='white', labelleft=0, labelbottom=0, top=1, right=1, length=ticklength, width=tickwidth, labelsize=fontsize-7)
    ...
    ax[1][1].text(1.5*spacing+0.005, 0.83*dx*ylen, '25 pc', color='white', fontsize=fontsize)
    ...
    mass_cloud_i = mass_cloud[time<=t]
    mass_out_cloud_i = mass_out_cloud[time_output<=t]

    ax[0][0].plot(time_i/1e3, mass_cloud_i/mass_cl_init, linewidth=linewidth, c="#49b4ab", label="total")
    ax[0][0].plot(time_output_i/1e3, mass_out_cloud_i/mass_cl_init, linewidth=linewidth-1, linestyle="--", c="#49b4ab", label="exited box")
    ...
    ax.set_xlim(0, tmax/1e3)
    ax.set_ylim(0-0.1, 1.0+0.1)

    ax.legend(loc="upper right", fontsize=fontsize-7)
    ax.tick_params(axis='both', which='both', direction='in', color=mode_color, top=1, right=1, length=9, width=2, labelsize=fontsize-7)
    ...
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(25,5), gridspec_kw={'wspace':0, 'hspace':0})
    ...
    ax.text(18*spacing, 0.83*dx*ylen, f'{round(t/1e3, 1)} Myr', color='white', fontsize=fontsize)
    ax.hlines(0.86*dx*ylen, 0.5*spacing, 1.5*spacing, color='white', linewidth=linewidth-1)
    ax.text(1.5*spacing+0.005, 0.83*dx*ylen, '25 pc', color='white', fontsize=fontsize)
    ax.text(0.65*spacing, 0.13*dx*ylen, 'dust', color='white', fontsize=fontsize)

    cbar_height = 0.77
    cbar_ax = fig.add_axes([0.9, 0.11, 0.025, cbar_height])
    cbar = fig.colorbar(im_g, cax=cbar_ax, pad=0.1)
    cbar.set_label(r'$\mathrm{log}_{10}(\Sigma_{dust})$ [$\mathrm{g}\,\mathrm{cm}^{-2}$]', rotation=270, labelpad=50, fontsize=fontsize-4)
    cbar.ax.tick_params(length=ticklength, width=tickwidth, color="white", labelsize=fontsize-7)
    cbar_spacing = (vlims_dust[1] - vlims_dust[0])/8
